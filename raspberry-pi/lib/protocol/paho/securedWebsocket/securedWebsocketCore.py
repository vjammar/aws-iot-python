...
sys.path.append("./lib/")
sys.path.append("../lib/")
from util.sigV4Core import sigV4Core
from exception.AWSIoTExceptions import wssNoKeyInEnvironmentError
from exception.AWSIoTExceptions import wssHandShakeError
...
class securedWebsocketCore:
    ...
    def __init__(self, socket, hostAddress, portNumber):
    ...
    def _verifyWSSResponse(self, response, clientKey):
        ...
        if "101 switching protocols" not in rawResponse or "upgrade: websocket" not in rawResponse or "connection: upgrade" not in rawResponse:
            return False
        # Parse out the sec-websocket-accept
        WSSAcceptKeyIndex = response.strip().index("sec-websocket-accept: ") + len("sec-websocket-accept: ")
        rawSecWebSocketAccept = response.strip()[WSSAcceptKeyIndex:].split("\r\n")[0].strip()
    ...
    def _verifyWSSAcceptKey(self, srcAcceptKey, clientKey):
        ...
        verifyServerAcceptKey = base64.b64encode((hashlib.sha1(clientKey + GUID)).digest())
    ...
    def _closeWssConnection(self):
        ...
        self._sslSocket.write(self._encodeFrame("", self._OP_CONNECTION_CLOSE, masked=1))
    ...
    def read(self, numberOfBytes):
        ...
        if not self._hasPayloadLengthFirst:  # Check if we need to buffer First Payload Length byte
            ...
            payloadLengthFirstByteArray = bytearray(payloadLengthFirst)
        ...
        if not self._hasPayloadLengthExtended:  # Check if we need to buffer Extended Payload Length bytes
            ...
            if self._payloadLengthBytesLength == 2:
                self._payloadLength = struct.unpack("!H", str(payloadLengthExtended))[0]
            else:  # _payloadLengthBytesLength == 8
                self._payloadLength = struct.unpack("!Q", str(payloadLengthExtended))[0]
